 ...
 */
export function recordWordsRead(bookId: string, words: number): void {
 ...
 */
export function recordChapterRead(bookId: string): void {
  const session = activeSessions.get(bookId);
  if (!session) return;
  session.chaptersRead += 1;
}

/**
 * End the current reading session and persist
 */
export async function endReadingSession(bookId: string): Promise<ReadingSession | null> {
  ...
  if (durationMs < 30000 && activeSession.chaptersRead === 0 && activeSession.wordsRead < 120) {
  ...
  const session: ReadingSession = {
    ...
    chaptersRead: activeSession.chaptersRead,
    wordsRead: activeSession.wordsRead,
  };
 ...
 */
export async function getWeeklyTrend(): Promise<{ date: string; minutes: number }[]> {
  const dailyData: Record<string, DailyReading> = FastStorage.getJSON(DAILY_READING_KEY) || {};
  const trend = [];
  const today = new Date();
  for (let i = 6; i >= 0; i--) {
