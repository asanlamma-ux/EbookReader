...
(function () {
  ...
  function extractClientRedirect(text, currentUrl) {
    var html = String(text || '');
    var match =
      html.match(/window\.location(?:\.replace|\.href)?\((['"])(.*?)\1\)/i) ||
      html.match(/window\.location\.href\s*=\s*(['"])(.*?)\1/i) ||
      html.match(/location\.replace\((['"])(.*?)\1\)/i) ||
      html.match(/<meta[^>]+http-equiv=(['"])refresh\1[^>]+content=(['"])[^'"]*url=([^'"]+)\2/i);
    var nextUrl = match ? (match[2] || match[3]) : '';
    if (!nextUrl) {
      var baseMatch = html.match(/var\s+redirect_link\s*=\s*(['"])(.*?)\1/i);
      if (baseMatch && baseMatch[2]) {
        nextUrl = baseMatch[2] + 'fp=-7';
      }
    }
    if (!nextUrl) return null;
    try {
      return new URL(nextUrl, currentUrl || location.href).toString();
    } catch (_error) {
      return nextUrl;
    }
  }

  async function fetchText(url, init, depth) {
    ...
    var redirectUrl = (depth || 0) >= 4 ? null : extractClientRedirect(text, resolvedUrl);
    if (redirectUrl && redirectUrl !== resolvedUrl) {
      return fetchText(redirectUrl, init, (depth || 0) + 1);
    }
    return text;
  }

  async function fetchTextWithMeta(url, init, depth) {
    var response = await fetch(url, Object.assign({
      credentials: 'include',
      headers: {
        accept: 'text/html,application/json,application/xhtml+xml',
    ...
    var redirectUrl = (depth || 0) >= 4 ? null : extractClientRedirect(text, resolvedUrl);
    if (redirectUrl && redirectUrl !== resolvedUrl) {
      return fetchTextWithMeta(redirectUrl, init, (depth || 0) + 1);
    }
    return {
      text: text,
      url: resolvedUrl,
  ...
  function parseSiteOrder(path) {
    var match = String(path || '').match(/(?:chapter[-_/]|_)(\d+)(?:\.html?)?(?:\/)?$/i);
    if (!match) {
      match = String(path || '').match(/\/(\d+)(?:\.html?)?(?:\/)?$/i);
    }
    return match ? Number(match[1]) : null;
  }
  ...
  function parseGenericTitle(doc) {
    var node =
      doc.querySelector('.r-n-bk-name, h1.booktitle, h1.tit, h1.novel-title, .novel-title h1, h1.title, h1') ||
      doc.querySelector('meta[property="og:title"]') ||
      doc.querySelector('meta[name="twitter:title"]');
    if (!node) return '';
  ...
  function parseGenericAuthor(doc) {
    var node =
      doc.querySelector('[itemprop="author"], .author a, .author span:last-child, .author, .book-author a, .novel-author, a[href*="/search/?author="], a[href*="/author/"]') ||
  ...
  function parseGenericSummary(doc) {
    var node =
      doc.querySelector('[itemprop="description"], .intro-content, .book-intro, .summary .content, .summary, .description, .synops, .synopsis, .bk-summary-txt, .book-summary-content') ||
      doc.querySelector('meta[property="og:description"]') ||
      doc.querySelector('meta[name="description"]');
    if (!node) return '';
  ...
  function parseGenericChapterCount(doc) {
    var candidates = [
      ...
      doc.querySelector('.chapter .s1, .s1'),
      doc.querySelector('.book-info .book-chapter, .book-chapter, .book-stats'),
    ];
    for (var i = 0; i < candidates.length; i += 1) {
  ...
  function parseGenericStatus(doc) {
    var node =
      doc.querySelector('.header-stats strong:last-child, .status, [class*="novel-status"], [class*="book-status"], .chapter .s2');
  ...
  async function fetchGenericNovelDetails(payload, providerId) {
    ...
    if (!chapters.length && providerId === 'novelcool') {
      var startLink = doc.querySelector('a.last-read-url[href], a[href*="/chapter/"]');
      if (startLink) {
        var startPath = parsePath(startLink.getAttribute('href') || '', providerId);
        chapters.push({
          order: 1,
          title: asText(
            (doc.querySelector('.r-n-bk-chp, .chapter-item-title') && doc.querySelector('.r-n-bk-chp, .chapter-item-title').textContent) ||
            'Full'
          ),
          path: startPath,
          updatedAt: null,
  ...
  async function searchGenericSite(payload, providerId) {
    ...
    if (payload && payload.cursor) {
      ...
      if (providerId === 'freewebnovel' && query) {
  ...
  async function fetchGenericChapter(payload, providerId) {
    ...
    var junk = contentNode.querySelectorAll('script, style, iframe, ins, [align="center"], .ads, .ad, .read-ads, .chapter-start, .chapter-end, .paging, .catalog');
    for (var i = 0; i < junk.length; i += 1) {
      if (junk[i] && junk[i].parentNode) junk[i].parentNode.removeChild(junk[i]);
    }
