...
type ScrollRestoreMode = 'saved' | 'savedChapterPercent' | 'chapterStart' | 'preserve';
type TtsChunk = { text: string; startIndex: number };
...
function estimateChapterPercentFromBookProgress(progress: number, chapterIndex: number, totalChapters: number): number {
  if (!totalChapters || totalChapters <= 0) return 0;
  const segmentSize = 100 / totalChapters;
  if (segmentSize <= 0) return 0;
  const progressWithinChapter = ((progress - chapterIndex * segmentSize) / segmentSize) * 100;
  return clampPercent(progressWithinChapter);
}
...
function stripHtmlToSpeechText(value: string): string {
  return String(value || '')
...
function chunkTextForTts(text: string, maxLength: number): TtsChunk[] {
  const normalized = String(text || '').trim();
  if (!normalized) return [];

  const limit = Math.max(240, Math.floor(maxLength || 0) || 3200);
  if (normalized.length <= limit) {
    return [{ text: normalized, startIndex: 0 }];
  }
  ...
  while (start < normalized.length) {
    let end = Math.min(normalized.length, start + limit);
    if (end < normalized.length) {
      const sentenceBreak = normalized.lastIndexOf('. ', end);
      const lineBreak = normalized.lastIndexOf('\n', end);
      const clauseBreak = normalized.lastIndexOf(', ', end);
      const wordBreak = normalized.lastIndexOf(' ', end);
      const bestBreak = [sentenceBreak, lineBreak, clauseBreak, wordBreak]
        .filter(index => index > start + Math.min(80, Math.floor(limit * 0.35)))
        .sort((a, b) => b - a)[0];

      if (typeof bestBreak === 'number') {
        end = bestBreak + 1;
      }
    }
    ...
    if (chunkText) {
      const offsetWithinSlice = normalized.slice(start, end).indexOf(chunkText);
      chunks.push({
        text: chunkText,
        startIndex: start + Math.max(0, offsetWithinSlice),
      });
    }

    start = end;
    while (start < normalized.length && /\s/.test(normalized.charAt(start))) {
      start += 1;
    }
  }
...
function replaceTermsInTextSegment(
  ...
  const accepted: { start: number; end: number; term: Term }[] = [];
  let consumedUntil = -1;
...
export default function ReaderScreen() {
  ...
  const [restoredChapterScrollPercent, setRestoredChapterScrollPercent] = useState(0);
  ...
  const [readerAiDraft, setReaderAiDraft] = useState('');
  ...
  const readerTouchStartRef = useRef<{ x: number; y: number; at: number } | null>(null);
  const pendingChapterAnchorRef = useRef<string | null>(null);
  const currentTtsTextRef = useRef('');
  ...
  const highestAppendedChapterRef = useRef(0);
  ...
  const lastReaderTapAtRef = useRef(0);
  const lastSelectionAtRef = useRef(0);
  const pendingSelectionClearTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  const translationLanguage = ((readingSettings as any).translationLanguage as string) || 'en';
  ...
  useEffect(() => {
    const loadBook = async () => {
      ...
      try {
        ...
        if (savedPosition && savedPosition.chapterIndex < parsed.chapters.length) {
          const savedChapterPercent = clampPercent(savedPosition.chapterScrollPercent);
          ...
          highestAppendedChapterRef.current = savedPosition.chapterIndex;
          ...
          setRestoredChapterScrollPercent(savedChapterPercent);
          setScrollRestoreMode(savedChapterPercent > 0 ? 'savedChapterPercent' : 'saved');
        } else if (foundBook.currentChapter > 0 && foundBook.currentChapter < parsed.chapters.length) {
          const fallbackChapterPercent = estimateChapterPercentFromBookProgress(
            foundBook.progress,
            foundBook.currentChapter,
            parsed.chapters.length
          );
          setCurrentChapterIndex(foundBook.currentChapter);
          setVisibleChapterIndex(foundBook.currentChapter);
          setLoadedChapterIndices([foundBook.currentChapter]);
          highestAppendedChapterRef.current = foundBook.currentChapter;
          setLastScrollPosition(0);
          setRestoredScrollPosition(0);
          setRestoredChapterScrollPercent(fallbackChapterPercent);
          setScrollRestoreMode(fallbackChapterPercent > 0 ? 'savedChapterPercent' : 'chapterStart');
        } else {
          setCurrentChapterIndex(0);
          setVisibleChapterIndex(0);
          setLoadedChapterIndices([0]);
          highestAppendedChapterRef.current = 0;
          setLastScrollPosition(0);
          setRestoredScrollPosition(0);
          setRestoredChapterScrollPercent(0);
    ...
    loadBook();
    // Intentionally keyed to the route id only.
    // updateBook() mutates library state, which can churn context callback identities
    // and cause this expensive parse/load routine to run in a loop.
  }, [id]);
  ...
  useEffect(() => {
    ...
    return () => {
      void endReadingSession(book.id);
    };
  }, [book?.id]);

  useEffect(() => {
    return () => {
      if (pendingSelectionClearTimerRef.current) {
        clearTimeout(pendingSelectionClearTimerRef.current);
  ...
  }, [chapterScrollPercent, lastScrollPosition, activeChapterIndex]);

  const persistCurrentReadingPosition = useCallback(async () => {
    if (!book?.id || !parsedEpub?.chapters.length) return;

    const totalChapterCount = parsedEpub.chapters.length;
    const chapterIndex = chapterIndexRef.current;
    const chapterPercent = clampPercent(chapterScrollPercentRef.current);
    const scrollPosition = Math.max(0, lastScrollPositionRef.current);
    const segmentSize = 100 / totalChapterCount;
    const progress = Math.min(100, Math.round(chapterIndex * segmentSize + (segmentSize * chapterPercent) / 100));
    const updates: Partial<Book> = { progress, currentChapter: chapterIndex };
    ...
    await saveReadingPosition({
      ...
      chapterIndex,
      scrollPosition,
      chapterScrollPercent: chapterPercent,
      timestamp: new Date().toISOString(),
    });
  }, [book?.id, book?.readingStatus, parsedEpub, saveReadingPosition, updateBook]);
  ...
  useEffect(() => {
    ...
    const langCode = translationLanguage;
    ...
    (async () => {
      ...
      if (cached) {
        setTranslatedChapters(prev => {
          ...
          next.set(idx, cached.translatedHtml);
          return next;
        });
  ...
  }, [currentChapterIndex, parsedEpub, autoTranslationMode, translationLanguage, id]);
  ...
  }, [showToolbar, progressOpacity, toolbarOpacity, toolbarTranslateY]);
  ...
  }, [id, activeChapterIndex, continuousReadingEnabled, loadedChapterIndices, getBookmarksByBook, getHighlightsByBook]);
  ...
  useEffect(() => {
    ...
    if (!continuousReadingEnabled) {
      ...
      highestAppendedChapterRef.current = currentChapterIndex;
      return;
    }
    ...
    setLoadedChapterIndices(prev => {
      ...
      const next = uniqueSortedChapterIndices([...prev, currentChapterIndex]);  
      const maxLoaded = next.length ? next[next.length - 1] : currentChapterIndex;
      highestAppendedChapterRef.current = maxLoaded;
      return next;
    });
  ...
  const openChapterAt = useCallback((
    ...
    const previousChapter = chapterIndexRef.current;

    if (book?.id && target !== previousChapter) {
      recordChapterRead(book.id);
    }
    ...
    if (continuousReadingEnabled && options?.keepLoaded) {
      ...
      if (target > highestAppendedChapterRef.current) {
        highestAppendedChapterRef.current = target;
      }
    } else {
    ...
    if (options?.preserveScroll) {
      setRestoredScrollPosition(lastScrollPositionRef.current);
      setRestoredChapterScrollPercent(chapterScrollPercentRef.current);
      setScrollRestoreMode('preserve');
    } else {
      setLastScrollPosition(0);
      setRestoredScrollPosition(0);
  ...
  }, [book?.id, continuousReadingEnabled, parsedEpub?.chapters.length]);
  ...
  }, [parsedEpub, activeChapterIndex, openChapterAt]);
  ...
  const handleTap = useCallback(
    async (x: number) => {
      ...
      if (Date.now() - lastSelectionAtRef.current < 700) return;
  ...
  const dispatchReaderTap = useCallback(
    (x: number) => {
      const now = Date.now();
      if (now - lastReaderTapAtRef.current < 250) return;
      lastReaderTapAtRef.current = now;
      void handleTap(x);
    },
    [handleTap]
  );
  ...
  const handleReaderTouchStart = useCallback((event: GestureResponderEvent) => {
    ...
    readerTouchStartRef.current = { x: pageX, y: pageY, at: Date.now() };
  }, []);
  ...
  const handleReaderTouchEnd = useCallback(
    (event: GestureResponderEvent) => {
      ...
      if (Date.now() - lastSelectionAtRef.current < 700) return;
      if (dx > 12 || dy > 12 || duration > 280) return;
      dispatchReaderTap(pageX);
    },
    [dispatchReaderTap]
  );
  ...
  const handleBack = useCallback(async () => {
    await persistCurrentReadingPosition();
    router.back();
  }, [persistCurrentReadingPosition, router]);
  ...
  }, [book?.id, parsedEpub, persistCurrentReadingPosition]);
  ...
  const handleHighlight = useCallback(async (data: HighlightData) => {
    ...
    `);
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
  }, [book, activeChapterIndex, addHighlight]);
  ...
  const handleBookmarkSelection = useCallback(async (text: string) => {
    ...
    Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
  }, [book, activeChapterIndex, lastScrollPosition, addBookmark]);
  ...
  const handleAskAiFromSelection = useCallback((text: string) => {
    ...
    setTermPopover(null);
    setSelection(null);
    setReaderAiDraft(`Help me understand this passage:\n\n"${selectedText}"`);
    setShowAiPanel(true);
    setShowToolbar(false);
  }, []);
  ...
  const generateReaderHTML = () => {
    ...
    const fontStack = getFontStack(typography.fontFamily);
    // The WebView source stays anchored to the active chapter only.
    // Continuous reading appends future chapters in-place so scroll-driven state
    // updates do not trigger a full document reload and wipe appended content.
    const renderedChapterIndices = [currentChapterIndex];
...
<head>
  ...
  <meta http-equiv="Content-Security-Policy" content="default-src 'none'; base-uri 'none'; form-action 'none'; frame-ancestors 'none'; connect-src 'none'; font-src data:; img-src data: blob:; media-src data: blob:; style-src 'unsafe-inline'; script-src 'unsafe-inline' 'sha256-fduvyJ5Xf8N0/PLzPgPzWWle4VfTg+jQ19kqlLAF6LA=';">
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
    ...
    html {
      ...
      scroll-behavior: ${readingSettings.readingFlowMode === 'scroll' ? 'smooth' : 'auto'};
    }
    ${readingSettings.blueLightFilter ? `
    html::after {
      content: '';
...
<body>
  ...
  <script>
    (function() {
      ...
      function clearTtsBoundary() {
        try {
          var activeMarks = document.querySelectorAll('[data-miyo-tts-active="true"]');
      ...
      function buildTtsCharMap(chapterIndex) {
        var chapterRoot =
          document.querySelector('.miyo-chapter-section[data-chapter-index="' + chapterIndex + '"] .miyo-chapter-content') ||
          document.querySelector('.miyo-chapter-section[data-chapter-index="' + chapterIndex + '"]') ||
          document.querySelector('.miyo-wrap') ||
          document.body;

        var walker = document.createTreeWalker(chapterRoot, NodeFilter.SHOW_TEXT, null);
        var chars = [];
        ...
        while ((node = walker.nextNode())) {
          ...
          for (var i = 0; i < text.length; i++) {
            ...
            if (isWhitespace) {
              ...
              chars.push({ node: node, start: i, end: i + 1, isWhitespace: true });
              lastWasWhitespace = true;
            } else {
              chars.push({ node: node, start: i, end: i + 1, isWhitespace: false });
              lastWasWhitespace = false;
            }
          }
        }

        while (chars.length && chars[chars.length - 1].isWhitespace) {
          chars.pop();
        }
      ...
      window.__clearTtsBoundary = function() {
        clearTtsBoundary();
      };

      window.__setTtsBoundary = function(charIndex, charLength, chapterIndex) {
        try {
          clearTtsBoundary();
          var chars = buildTtsCharMap(chapterIndex);
          if (!chars.length) return;

          var startIndex = Math.max(0, Math.floor(Number(charIndex) || 0));
          var spanLength = Math.max(1, Math.floor(Number(charLength) || 1));
          if (startIndex >= chars.length) return;

          var startInfo = chars[startIndex];
          var endInfo = chars[Math.min(chars.length - 1, startIndex + spanLength - 1)];
          ...
          mark.className = 'miyo-tts-word';
          mark.setAttribute('data-miyo-tts-active', 'true');
          var fragment = range.extractContents();
          mark.appendChild(fragment);
          range.insertNode(mark);

          var rect = mark.getBoundingClientRect();
          if (rect && (rect.top < 72 || rect.bottom > window.innerHeight - 120)) {
      ...
      var selTimeout;
      var lastSelectionNotifyAt = 0;
      function selectionGraceActive() {
        return Date.now() - lastSelectionNotifyAt < 700;
      }
      ...
      function computeChapterMetrics() {
        ...
        var maxScrollWithinChapter = Math.max(1, end - start - window.innerHeight);
        var chapterOffset = Math.max(0, Math.min(window.scrollY - start, maxScrollWithinChapter));
        var percent = Math.max(0, Math.min(100, (chapterOffset / maxScrollWithinChapter) * 100));
        return {
...
// Tap and Swipe zone tracking
      ...
      document.addEventListener('touchend', function(e) {
        ...
        if (selectionGraceActive()) return;
  ...
  const handleWebViewMessage = useCallback((event: any) => {
    try {
      ...
      if (data.type === 'selection') {
        ...
        if (Date.now() - lastSelectionAtRef.current < 400) {
          return;
        }
        if (pendingSelectionClearTimerRef.current) {
          clearTimeout(pendingSelectionClearTimerRef.current);
        }
        pendingSelectionClearTimerRef.current = setTimeout(() => {
          setSelection(null);
          pendingSelectionClearTimerRef.current = null;
        }, 80);
        ...
        if (data.scrollY !== undefined) {
          ...
          lastScrollPositionRef.current = data.scrollY;
        }
        if (typeof data.chapterIndex === 'number' && !Number.isNaN(data.chapterIndex)) {
          ...
          if (!continuousReadingEnabled) {
            setCurrentChapterIndex(nextVisibleChapter);
          }
          chapterIndexRef.current = nextVisibleChapter;
        }
        if (typeof data.chapterScrollPercent === 'number' && !Number.isNaN(data.chapterScrollPercent)) {
          const clampedPercent = Math.min(100, Math.max(0, data.chapterScrollPercent));
          setChapterScrollPercent(clampedPercent);
          chapterScrollPercentRef.current = clampedPercent;
        } else if (typeof data.scrollPercent === 'number' && !Number.isNaN(data.scrollPercent)) {
          const clampedPercent = Math.min(100, Math.max(0, data.scrollPercent));
          setChapterScrollPercent(clampedPercent);
          chapterScrollPercentRef.current = clampedPercent;
        }
        ...
        if (
          ...
          continuousReadingEnabled &&
          parsedEpub &&
          highestAppendedChapterRef.current < parsedEpub.chapters.length - 1
        ) {
          if (!bottomChapterNavLockRef.current) {
            bottomChapterNavLockRef.current = true;
            setTimeout(() => { bottomChapterNavLockRef.current = false; }, 3000);
            const nextIndex = Math.min(parsedEpub.chapters.length - 1, highestAppendedChapterRef.current + 1);
            const appendedInPlace = appendChapterToReader(nextIndex);
            if (appendedInPlace) {
              highestAppendedChapterRef.current = nextIndex;
            }
            setLoadedChapterIndices(prev => {
  ...
  const currentChapterSpeechText = useMemo(() => {
    const sourceContent =
      showTranslated && translatedChapters.has(activeChapterIndex)
        ? translatedChapters.get(activeChapterIndex) || currentChapter?.content || ''
        : currentChapter?.content || '';
    const renderedContent = replaceTermsInHtmlContent(
      sourceContent,
      appliedTerms,
      activeChapterIndex
    );
    return stripHtmlToSpeechText(renderedContent);
  }, [
    activeChapterIndex,
    appliedTerms,
    currentChapter?.content,
  ...
  const sleepMinutesLeft = useMemo(() => {
    ...
    return Math.max(0, Math.ceil((sleepDeadlineMs - Date.now() + sleepUiTick * 0) / 60000));
  ...
  const clearTtsBoundary = useCallback(() => {
    webViewRef.current?.injectJavaScript(`
(function() {
  if (window.__clearTtsBoundary) {
    window.__clearTtsBoundary();
  }
})();
true;
`);
  ...
  const syncTtsBoundary = useCallback(
    (charIndex: number, charLength: number) => {
      const safeIndex = Math.max(0, Math.floor(charIndex));
      const safeLength = Math.max(1, Math.floor(charLength || 1));
      webViewRef.current?.injectJavaScript(`
(function() {
  if (window.__setTtsBoundary) {
    window.__setTtsBoundary(${safeIndex}, ${safeLength}, ${activeChapterIndex});
  }
})();
true;
`);
  ...
  const startTts = useCallback(() => {
    if (!currentChapterSpeechText) return;
    stopTts();
    currentTtsTextRef.current = currentChapterSpeechText;
    setTtsPlaying(true);
    clearTtsBoundary();
    ...
    const chunks = chunkTextForTts(
      currentChapterSpeechText,
      Math.max(1200, Math.min(Speech.maxSpeechInputLength || 3200, 3500))
    );
    ...
    const speakChunk = (chunkIndex: number) => {
      ...
      Speech.speak(chunk.text, {
        ...
        onBoundary: (event: { charIndex: number; charLength: number }) => {
          if (ttsSessionIdRef.current !== sessionId) return;
          syncTtsBoundary(
            chunk.startIndex + Math.max(0, event?.charIndex || 0),
            Math.max(1, event?.charLength || 1)
          );
        },
        onDone: () => {
          if (ttsSessionIdRef.current !== sessionId) return;
    ...
    speakChunk(0);
  }, [clearTtsBoundary, currentChapterSpeechText, stopTts, syncTtsBoundary]);
...
`;
  ...
  useEffect(() => {
    ...
    if (currentTtsTextRef.current !== currentChapterSpeechText) {
      stopTts();
    }
  }, [currentChapterSpeechText, stopTts, ttsPlaying]);

  const bookHighlights = useMemo(
    () => (book ? getHighlightsByBook(book.id) : []),
    [book, getHighlightsByBook]
  );
  const allBookmarks = useMemo(
    () => (book ? getBookmarksByBook(book.id) : []),
    [book, getBookmarksByBook]
  );
  const chapterTitles = useMemo(
    () => parsedEpub?.chapters.map(c => c.title) || [],
    [parsedEpub]
  );
  ...
  return (
    <View style={[styles.container, { backgroundColor: currentTheme.background }]}>
      ...
      <Animated.View
        ...
        style={[styles.topToolbar, toolbarAnimatedStyle, { paddingTop: insets.top }]}
      >
            ...
            {readingSettings.autoTranslationMode !== 'off' && (
              <PressableScale
                ...
                style={[
                  ...
                  ...(showTranslated ? [{ backgroundColor: currentTheme.accent + '22', borderRadius: 8 }] : []),
                ]}
              >
                {isTranslating ? (
      ...
      {!selection && (
        <Animated.View
          ...
          style={[
            styles.aiFloatingButtonWrap,
            {
              right: 18,
              bottom: showToolbar ? insets.bottom + 118 : insets.bottom + 18,
            },
          ]}
        >
          <PressableScale
            ...
            style={[
              ...
              {
                backgroundColor: showToolbar ? currentTheme.background + 'F2' : currentTheme.background + 'B3',
                borderColor: currentTheme.secondaryText + '22',
              },
            ]}
          >
            <Sparkles size={18} color={currentTheme.accent} strokeWidth={2} />
          </PressableScale>
        </Animated.View>
      )}
      ...
      <Animated.View
        pointerEvents={showToolbar ? 'auto' : 'none'}
        style={[styles.bottomToolbar, bottomToolbarAnimatedStyle, { paddingBottom: insets.bottom + 8 }]}
      >
        <View style={[styles.toolbarBackground, { backgroundColor: currentTheme.background + 'F2' }]}>
          <View style={styles.bottomToolbarInner}>
            ...
            >
              ...
              <PressableScale
                ...
                style={styles.actionBtn}
              >
                {ttsPlaying ? <Square size={20} color={currentTheme.accent} strokeWidth={2} /> : <Play size={20} color={currentTheme.text} strokeWidth={2} />}
                <ThemedText variant="secondary" size="caption" style={styles.actionLabel}>{ttsPlaying ? 'Stop TTS' : 'TTS'}</ThemedText>
              </PressableScale>
      ...
      {showAiPanel ? (
        <ReaderAiPanel
          ...
          chapterHtml={showTranslated && translatedChapters.has(activeChapterIndex)
            ? translatedChapters.get(activeChapterIndex) || currentChapter?.content || ''
            : currentChapter?.content || ''}
          chapters={parsedEpub?.chapters.map(chapter => ({
            title: chapter.title,
            html: chapter.content.slice(0, 6000),
          })) || []}
          initialPrompt={readerAiDraft}
        />
      ) : null}
