...
interface TermsContextType {
  ...
  canPublishCommunityGroups: () => Promise<boolean>;
...
export function TermsProvider({ children }: { children: ReactNode }) {
  ...
  const canPublishCommunityGroups = useCallback(async (): Promise<boolean> => {
    try {
      const {
        data: { user },
      ...
      if (normalizedRole === 'admin' || normalizedRole === 'verified_member' || normalizedRole === 'verified') {
        return true;
      }
      ...
      return profileRole === 'admin' || profileRole === 'verified_member' || profileRole === 'verified' || !!data?.is_verified;
    } catch {
      return false;
    }
  }, []);
