...
const READING_POSITIONS_FAST_KEY = 'miyo.reading-positions';
...
export function LibraryProvider({ children }: { children: ReactNode }) {
  ...
  const loadData = async () => {
    try {
      ...
      if (positionsJson) {
        const parsedPositions = JSON.parse(positionsJson);
        setReadingPositions(parsedPositions);
        FastStorage.setJSON(READING_POSITIONS_FAST_KEY, parsedPositions);
      }
  ...
  const saveReadingPosition = useCallback(async (position: ReadingPosition) => {
    setReadingPositions(prev => {
      ...
      FastStorage.setJSON(READING_POSITIONS_FAST_KEY, newPositions);
      return newPositions;
    });
  ...
  const getReadingPosition = async (bookId: string) => {
    if (readingPositions[bookId]) {
      return readingPositions[bookId];
    }

    const cachedPositions = FastStorage.getJSON<Record<string, ReadingPosition>>(READING_POSITIONS_FAST_KEY);
    if (cachedPositions?.[bookId]) {
      setReadingPositions(current => (current[bookId] ? current : cachedPositions));
      return cachedPositions[bookId];
    }
    ...
    try {
      ...
      FastStorage.setJSON(READING_POSITIONS_FAST_KEY, parsed);
      setReadingPositions(current => (Object.keys(current).length ? current : parsed));
      return parsed[bookId] || null;
    } catch (error) {
      captureError('Get Reading Position', error);
      return null;
    }
  };
