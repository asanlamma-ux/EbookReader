...
export function CommunityTermGroupsModal({ visible, onClose }: CommunityTermGroupsModalProps) {
  ...
  const { fetchCommunityGroups, downloadCommunityGroup, canPublishCommunityGroups, termGroups } = useTerms();
  ...
  useEffect(() => {
    if (visible) {
      loadGroups();
      canPublishCommunityGroups().then(setCanPublish).catch(() => setCanPublish(false));
    }
  }, [visible, canPublishCommunityGroups, loadGroups]);
    ...
    <Modal visible={visible} transparent animationType="none" onRequestClose={onClose} statusBarTranslucent>
      <View style={[styles.root, { backgroundColor: 'rgba(0,0,0,0.5)' }]}>
        ...
        >
          ...
          {canPublish ? (
            <View style={[styles.publishBanner, { backgroundColor: currentTheme.accent + '14', borderColor: currentTheme.accent + '35' }]}>
              <Crown size={16} color={currentTheme.accent} />
              <View style={{ flex: 1 }}>
                <ThemedText variant="accent" size="caption" weight="semibold">
                  Verified publishing unlocked
                </ThemedText>
                <ThemedText variant="secondary" size="caption">
                  This account can publish shared term groups once the create flow is wired to a secured Supabase RPC.
                </ThemedText>
              </View>
            </View>
          ) : (
            <View style={[styles.publishBanner, { backgroundColor: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.025)', borderColor: currentTheme.secondaryText + '18' }]}>
              <Info size={16} color={currentTheme.secondaryText} />
              <View style={{ flex: 1 }}>
                <ThemedText variant="primary" size="caption" weight="semibold">
                  Publishing is restricted
                </ThemedText>
                <ThemedText variant="secondary" size="caption">
                  Only verified members or admins should be allowed to publish shared term groups.
                </ThemedText>
              </View>
            </View>
          )}
