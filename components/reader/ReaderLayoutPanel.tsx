  ...
  return (
    <>
      ...
      >
        ...
        <View style={styles.section}>
          <View style={styles.sectionLabel}>
            <ScrollText size={16} color={currentTheme.secondaryText} />
            <ThemedText variant="secondary" size="caption" weight="semibold">
              READING FLOW
            </ThemedText>
          </View>
          <ThemedText variant="secondary" size="caption" style={{ marginBottom: 10, opacity: 0.9 }}>
            Scroll for web-novel style reading, or paged for chapter-by-chapter focus.
          </ThemedText>
          <View style={styles.chipRow}>
            {FLOW_OPTIONS.map(opt => {
              const active = readingSettings.readingFlowMode === opt.id;
              return (
                <PressableScale
                  key={opt.id}
                  onPress={() => setReadingSettings({ readingFlowMode: opt.id })}
                  style={[
                    styles.chip,
                    {
                      backgroundColor: active ? currentTheme.accent + '22' : currentTheme.background,
